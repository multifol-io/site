...
string wikiLinksPWFile = "c:\\temp\\wikilinksPW.txt";

var client = new WikiClient();
SiteOptions options = new() { AccountAssertion = AccountAssertionBehavior.AssertBot, ApiEndpoint = wikiApiUrl };
string? password;

if (File.Exists(wikiLinksPWFile))
{
    password = await File.ReadAllTextAsync(wikiLinksPWFile);
}
else
{
    Console.WriteLine("couldn't find: " + wikiLinksPWFile);
    return;
}

var wikiSite = new WikiSite(client, options, "RobRelyea", password);
